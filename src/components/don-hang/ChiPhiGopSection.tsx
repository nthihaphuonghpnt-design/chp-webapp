...
export default function ChiPhiGopSection({
  ...
  async function luuDongMoi(row: { key: number } & NewRowValues) {
    const nhanVienId = await layNhanVienId();
    if (row.loai === "chi_phi") {
  ...
  function renderEditCells(loai: Loai, values: NewRowValues, set: (patch: Partial<NewRowValues>) => void, onlySellEditable: boolean) {
          ...
          <tbody>
                ...
                <tr key={rv.key} className="border-t border-slate-100 align-top">
                  {dangSua ? (
                    <>
                      ...
                      {renderEditCells(rv.loai, editValues!, (patch) => setEditValues((prev) => (prev ? { ...prev, ...patch } : prev)), chiSuaSell)}
                      <td className="px-2 py-1.5">
            ...
            {addingRows.map((r) => (
              <tr key={r.key} className="border-t border-slate-100 bg-blue-50/30 align-top">
                ...
                {renderEditCells(r.loai, r, (patch) => capNhatDongMoi(r.key, patch), false)}
